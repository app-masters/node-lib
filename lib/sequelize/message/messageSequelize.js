...
class MessageSequelize {
  ...
  static setup(transportConfig: Object, messagesConfig: Object): void {
    if (transportConfig) this.setTransportConfig(transportConfig);
    if (messagesConfig) this.setMessagesConfig(messagesConfig);
  ...
  static setTransportConfig(transportConfig) {
    ...
    if (!transportConfig.host)
      throw new Error(
        "First MessageSequelize param must just transportConfig. Received:" +
  ...
  static setMessagesConfig(messagesConfig) {
    ...
    if (messagesConfig.host || messagesConfig.mail)
      throw new Error(
        "Secong MessageSequelize param must have just messages, not the merged with transportConfig. Received: " +
          JSON.stringify(messagesConfig)
      );
    // Set it
  ...
  static async sendHtmlMessage(
    ...
    if (!transportConfig)
      throw new Error(
        "A transportConfig must be passed on MessageSequelize.sendHtmlMessage forth parameter"
      );
    if (!toMail || !subject || !bodyHtml)
      throw new Error(
        "You must pass toMail, subject and bodyHtml on MessageSequelize.sendHtmlMessage"
      );
    ...
    // Saving message to get the ID on html
    await messageInstance.save();
    replaceFields.messageIdHashed = md5(messageInstance._id.toString());

    messageInstance.setBodyHtml(
      MessageSequelize.parseString(bodyHtml, replaceFields, true)
    );
    await messageInstance.save();

    if (sendToQueue) return messageInstance;
   ...
   * @returns {Promise.<*>}
   */
  static async sendObjectMessage(
    messageKey: string,
    toMail: string,
    fromUserId: number,
    ...
    if (!messageKey)
      throw new Error(
        "You must pass messageKey to MessageSequelize.sendHtmlMessage"
      );
    ...
    const messageInstance = new MessageInstance({
      ...
      subject: MessageSequelize.parseString(
  ...
  static applyTemplate(
    ...
    if (MessageSequelize.template) {
      const button = MessageSequelize.getButtonFromTemplate(
        actions,
        MessageSequelize.template.buttonTemplate
      );
      const mustParse = {
        ...replaceFields,
        upperText: bodyHtml,
      ...
      bodyHtml = MessageSequelize.parseString(
        MessageSequelize.template.layout,
        mustParse
      );
    }
    return bodyHtml;
  ...
  static parseString(value: string, fields: Object, shouldCleanHtml: boolean) {
    return value
      .split(/{{([\s\S]*?)}}/)
      .map((t, i) =>
        i % 2 !== 0 ? fields[t] || (shouldCleanHtml ? "" : "{{" + t + "}}") : t
      )
      .join("");
  }
  ...
  static objectTostring(obj: Object, config: Object) {
    ...
    Object.keys(obj).map(key => {
      ...
      newKey =
        key.toLowerCase() !== "uppertext" && key.toLowerCase() !== "lowertext"
          ? `<b> ${newKey}: </b> `
          : "";
   ...
   * @returns {string}
   */
  static getButtonFromTemplate(
    actions: Array<Object>,
    buttonTemplate: string
  ): string {
    return (actions || [])
      .map(act => MessageSequelize.parseString(buttonTemplate, act))
      .join("");
  }
   ...
   */
  static async getByReceiverAndSubject(toMail: string, subject: string) {
    const MessageRepository = require("./messageRepository");
    return MessageRepository.getByReceiverAndSubject(toMail, subject);
  }

  /**
   * Set a message as read by the hash of it's _id
   * @param hashedId
   * @returns {*}
   */
  static async setReadByHash(hashedId: string) {
    const MessageRepository = require("./messageRepository");
    return MessageRepository.setReadByHash(hashedId);
  }

  /**
   * Check if a message was sent to some email address
   * @param messageKey
   * @param toMail
   * @return {Promise<Boolean>}
   */
  static async checkMessageWasSent(
    messageKey: string,
    toMail: string
  ): Promise<Boolean> {
    const messageConfig = MessageSequelize.getMessageConfig(messageKey);
    ...
    const MessageRepository = require("./messageRepository");
    const messages = await MessageRepository.getByReceiverAndSubject(
      toMail,
      messageConfig.subject
    );
