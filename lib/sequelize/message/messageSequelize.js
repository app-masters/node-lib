...
class MessageSequelize {
    ...
    static async sendHtmlMessage(toMail, subject, bodyHtml, fromUserId, replaceFields, transportConfig) {
        ...
        bodyHtml = this.applyTemplate(toMail, bodyHtml, replaceFields);
        ...
        messageInstance.setBodyHtml(MessageSequelize.parseString(bodyHtml, replaceFields));
    ...
    static async sendObjectMessage(messageKey: string, toMail: string, fromUserId, replaceFields, transportConfig: Object) {
        ...
        // Construct bodyHtml with template
        bodyHtml = this.applyTemplate(toMail, bodyHtml, replaceFields, messageConfig.actions);
     ...
     * @returns {*}
     */
    static applyTemplate(toMail, bodyHtml, replaceFields, actions) {
    ...
    static parseString(value: string, fields: Object) {
        return value.split(/{{([\s\S]*?)}}/).map((t, i) => (i % 2) !== 0 ? (fields[t] || t) : t).join('');
    }
