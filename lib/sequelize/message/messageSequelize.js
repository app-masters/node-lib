...
class MessageSequelize {
    ...
    static async sendHtmlMessage(toMail, subject, bodyHtml, fromUserId, replaceFields, transportConfig) {
        ...
        bodyHtml = this.applyTemplate(toMail, bodyHtml, replaceFields, transportConfig);
        ...
        messageInstance.setBodyHtml(bodyHtml);
     ...
     * @returns {Promise.<*>}
     */
    static async sendObjectMessage(messageKey: string, content: Object, toMail: string, fromUserId, replaceFields, transportConfig: Object) {
        if (!transportConfig)
            throw new Error('A transportConfig must be passed on MessageSequelize.sendHtmlMessage forth parameter');
        if (!messageKey || !content)
            throw new Error('You must pass messageKey and content MessageSequelize.sendHtmlMessage');
        ...
        let bodyHtml = MessageSequelize.objToHtml(content, {...messageConfig, fields: {...messageConfig.fields}});
     ...
     * @returns {*}
     */
    static applyTemplate(toMail, bodyHtml, replaceFields, transportConfig) {
        if (MessageSequelize.template) {

            const button = MessageSequelize.getButtonFromTemplate(MessageSequelize.template.actions, MessageSequelize.template.buttonTemplate);
            const mustParse = {
                ...replaceFields,
                upperText: bodyHtml,
                button
    ...
    static objToHtml(content: Object | string, config: Object) {
        ...
        if (typeof content === 'string') {
            return MessageSequelize.parseString(content, config);
        }
    }

    /** Helper to objToHtml **/
    static parseString(value: string, fields: Object) {
        return value.split(/{{([\s\S]*?)}}/).map((t, i) => (i % 2) !== 0 ? (fields[t] || ' ') : t).join('');
    }
