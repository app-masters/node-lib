...
class MessageSequelize {
     ...
     * @returns {Promise.<*>}
     */
    static async sendObjectMessage (messageKey: string, toMail: string, fromUserId: number, replaceFields: Object, transportConfig: Object, sendToQueue: boolean) {
        if (!transportConfig)
            throw new Error('A transportConfig must be passed on MessageSequelize.sendHtmlMessage forth parameter');
        if (!messageKey)
     ...
     */
    static async checkMessageWasSent(messageKey: string, toMail: string) : Promise<Boolean> {
        const messageConfig = MessageSequelize.getMessageConfig(messageKey);
        if (!messageConfig) {
            throw new Error('messageKey not found: ' + messageKey);
        }
        ...
        const MessageRepository = require('./messageRepository');
        const messages = await MessageRepository.getByReceiverAndSubject(toMail, messageConfig.subject);
