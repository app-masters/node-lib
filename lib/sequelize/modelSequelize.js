...
class ModelSequelize extends Model {
    ...
    async find(where, include, order, limit, attr) {
        try {
            let options = {where};
            if (include) {
    ...
    /**
     * Return an array of instanceClass from string raw sql query
     * @returns {Promise.<>}
     */
    async query(sql: string, instanceClass) {
        let [result] = await SequelizeInstance.getInstance().query(sql);
        // console.log("result", result);
        return result.map((item) => new instanceClass(item));
    }

    /**
     * Return one instanceClass from string raw sql query
     * @returns {Promise.<>}
     */
    async queryOne(sql: string, instanceClass) {
        let [result] = await SequelizeInstance.getInstance().query(sql);
        // console.log("result", result);
        return new instanceClass(result[0]);
    }
