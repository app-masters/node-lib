...
class ModelSequelize extends Model {
    ...
    async save(obj) {
        try {
            ...
            if (obj._id) {
                result = this.updateOne({_id: obj._id}, obj).then(item => item.get({plain: true})).catch(e => this.defaultCatch);
            } else {
                result = this.create(obj).then(item => item.get({plain: true})).catch(e => this.defaultCatch);
            }
            return result;
        } catch (e) {
            console.dir(e);
    ...
    find(where, include, order) {
        ...
        return this.model.findAll(options).then(items => items.map((item) => this.returnItemInstance(item))).catch(e => this.defaultCatch);
    }
    ...
    findById(id, include) {
        ...
        return this.model.findById(id, options).then((item) => this.returnItemInstance(item)).catch(e => this.defaultCatch);
    }
    ...
    findOne(where, include) {
        ...
        return this.model.find(options).then((item) => this.returnItemInstance(item)).catch(e => this.defaultCatch);
    }

    update(where, obj) {
        let options = {where};
        return this.model.update(obj, options).then(response => !!response[0]).catch(e => this.defaultCatch);
    }
