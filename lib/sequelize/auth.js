...
class Auth {
    ...
    async socialLogin (socialUser, siteName) {
        if (!socialUser.name && siteName === 'facebook') {
            throw new Error('User without name on socialLogin');
        }
        if (!socialUser.id || !socialUser.token || !socialUser.email) {
            throw new Error('User without id, token or email on socialLogin');
        }
