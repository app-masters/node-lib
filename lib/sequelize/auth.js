...
class Auth {
    ...
    /// Facebook (second version) - #needrefactory #repeatedcode
    async facebookLoginRoute (req, res, next) {
        const user = await Auth.facebookLogin(req.body);
        let userData = Auth.getUserData(user);
        req.login(user, function (err) {
    ...
    static async facebookLogin (facebookUser) {
        if (!facebookUser.id || !facebookUser.token || !facebookUser.email || !facebookUser.name) {
            return next(new Error('id, token, email and name needed'));
        }
        // Search for that user
        let user = await Auth.UserModel.findOne({'facebookId': facebookUser.id});

        if (!user) {
            // User not found - search in local by email
            user = await Auth.UserModel.findOne({'localEmail': facebookUser.email.toLowerCase()});
            if (!user) {
                const UserInstance = Auth.UserModel.getItemInstance();
                user = new UserInstance();
                user.name = facebookUser.name;
                user.local.email = facebookUser.email.toLowerCase();
            }
