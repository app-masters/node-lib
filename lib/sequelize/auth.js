...
class Auth {
    ...
    static checkPassword (user, foundUser) {
        const password = bcrypt.hashSync(user.localPassword, bcrypt.genSaltSync(8), null);
        return;
    }
    ...
    async facebookLogin (req, res, next) {
        ...
        let user = await Auth.UserModel.findOne({'facebookId': req.body.id});

        if (!user) {
            // User not found - search in local by email
            user = await Auth.UserModel.findOne({'localEmail': req.body.email.toLowerCase()});
            if (!user) {
                user = new Auth.UserModel.getItemInstance();
