...
class DateTime {
    ...
    static fillIntervalData (data, period, value) {
        ...
        data = data.map(d =>
            Object.assign(d, {value: d.value.split('-')[0]}));
        console.log('data 2', data, values);
        values = values.map((name, i) => Object.assign(data.find(v => Number(v.value) === i + 1) || {},
            {
                name,
                value: (data.find(v => Number(v.value) === i + 1) || {count: 0}).count}
        ));
        console.log(values);
        return values;
    }
