...
class AMError {
    static init(app) {
        ...
        if (config.rollbar && (process.env.NODE_ENV !== 'development' || config.rollbar.logOnDev)) {
            ...
            global.rollbar = {
                log: (message, obj) => {
                    console.log('> Should be sent to rollbar', message, obj);
                },
                error: (message, obj) => {
                    console.error('> Should be sent to rollbar', message, obj);
                },
                info: (message, obj) => {
                    console.info('> Should be sent to rollbar', message, obj);
                },
                warn: (message, obj) => {
                    console.warn('> Should be sent to rollbar', message, obj);
                }
            };
    ...
    static listen(app) {
        ...
        if (global.rollbarDisabled) {
            console.error("API on production cannot run without rollbar token. Exiting start");
            process.exit(1);
            return false;
        }
