...
class AMAuth {
    ...
    _localSignupStrategy(req, email, password, done) {
        this.sign({local:{email,password}}).then(user=>{
    ...
    _jwtStrategy(payload, done) {
        // console.log('_jwtStrategy', payload);
    ...
    async signup(email, password, cb) {
        this._localSignupStrategy(null, email.toLowerCase(), password, cb);
    }

    async signupUser(req,res,next){
        let user = req.body.user;
        try{
            user = await this.sign(user);
            ...
            AMAuth.UserModel.findOne({'local.email': email.toLowerCase()}, function (err, user) {
                ...
                if (!user) {
                    ...
                    if (AMAuth.securityConfig.singleLoginSignup === true) {
                        return this._localSignupStrategy(req, email.toLowerCase(), password, done);
                    } else {
    ...
    requireAuth(req, res, next) {
        ...
        return AMAuth.passport.authenticate('jwt', {session: false}, function (err, user, info) {
            ...
            if (user) {
                ...
                this.certifyLastAtributes(req.user, req.headers);
                return next();
            } else {
     ...
     * @returns {void} 
     */
    certifyLastAtributes(user, headers){
        let changed = false
        ...
        if(!user.initialClient || !user.initialClientVersion) {
            ...
            user.initialClientVersion = headers['client-version'];
            changed = true;
        }
        if(user.lastClient != headers['client'] || user.lastClientVersion != headers['client-version']){
            user.lastClient = headers['client'];
            user.lastClientVersion = headers['client-version'];
            changed = true;
        }
        if(changed){
