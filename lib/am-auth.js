...
class AMAuth {
    ...
    _localLoginStrategy(req, email, password, done) {
        ...
        process.nextTick(function () {
            ...
            AMAuth.UserModel.findOne({'local.email': email}, function (err, user) {
                ...
                if (!user) {
                    // singleLoginSignup
                    if (AMAuth.securityConfig.singleLoginSignup){
                        return AMAuth._localSignupStrategy(req,email, password, done);
                    } else {
    ...
    async signup(email, password, cb) {
        AMAuth._localSignupStrategy(null,email, password, cb);
    }
    ...
    localLogin(req, res, next) {
        try {
            console.log('localLogin req', req);
