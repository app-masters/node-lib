...
class nodeRestful {
    ...
    static registerRoute(app, router, route) {
        ...
        if (route.extraRoutes){
            for (let extraRoute of route.extraRoutes) {
                console.log(extraRoute);
                resource.route(extraRoute.route, {detail: extraRoute.detail, handler: extraRoute.handler});
            }
        }
        ...
        if (route.afterDelete) resource.after('delete', route.afterDelete);
        ...
        if (route.beforeDelete) resource.before('delete', route.beforeDelete);
