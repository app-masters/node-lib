...
class ModelSequelize extends Model {
    static setup (sequelize, modelName, schema, itemInstance, relationArray, modelOptions) {
        if (!modelName) {
        ...
        if (relationArray) {
            for (const relation of relationArray) {
                if (checkModel(relation.model)) {
                    model.belongsTo(relation.model.model, relation.options);
                }
            }
        }
    }
