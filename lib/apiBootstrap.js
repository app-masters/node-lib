...
function setup(app, envs, packag, passport) {
    ...
    if (envs.production && envs.production.database && envs.production.database.url)
        throw new Error("Database URL are not allowed in prod config anymore. Use heroku env DATABASE_URL to store it.");
    if (envs.staging && envs.staging.database && envs.staging.database.url)
        throw new Error("Database URL are not allowed in staging config anymore. Use heroku env DATABASE_URL to store it.");
    if (!envs.development)
        throw new Error("You should pass the complete 'envs' objects as second parameter to amBootstrap, not more 'config'. Bootstrap will load the right env config.");
    ...
    if (nodeEnv === "production" || nodeEnv === "staging") {
        if (!process.env.DATABASE_URL)
            throw new Error("Enviroment variable DATABASE_URL are not set");
    ...
    if (!config.database.url)
        throw new Error("config.database.url not set on current env \"" + nodeEnv + "\"");
